...
```{r} 
# example showing how the conditioning of the matrix changes based on choice of multiplier
set.seed(12)
reg <- get_regression_model(n = 200, d = 40, a = 4, num_true_signals = 8)
sapply(c(2, 1.98, 1.96, 1.9, 1.8), function(mult) condition_number_test_equi_mult(reg$X, multiplier = mult))
```

```{r}
# example showing how much this changes the D matrix
set.seed(12)
reg <- get_regression_model(n = 200, d = 40, a = 4, num_true_signals = 8)
unique(diag(create_equicorrelated_mult(reg$X, multiplier = 2)$D))
unique(diag(create_equicorrelated_mult(reg$X, multiplier = 1.9)$D))
```
