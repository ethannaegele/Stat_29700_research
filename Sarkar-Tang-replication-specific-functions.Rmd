...
get_T_1 <- function(X, Y, equicor = TRUE){
  sigma <- t(X) %*% X
  if (equicor){
  equi <- create_equicorrelated_mult(X, multiplier = 1.98) # TRYING TO USE THE MODIFIED FUNCTION FOR BETTER MATRIX CONDITION
  D <- equi$D # need to turn the output into a diagonal matrix
  X_ko <- equi$Xk
    #D <- diag(create.solve_equi(sigma)) # this appears to give 0 power?
   # X_ko <- create.fixed(X, method = 'equi')$Xk 
  }
  else{
    D <- diag(create.solve_sdp(sigma) + rnorm(ncol(X), mean = 0, sd = .01))
    X_ko <- create.fixed(X, method = 'sdp')$Xk
  }
...
get_T_2 <- function(X, Y, equicor = TRUE){
  if (equicor){
    D <- diag(create.solve_equi(t(X) %*% X)) # note: D calculated in slightly different ways for T1 and T2; this does not seem to make much of a difference in the overall results
  X_ko <- create.fixed(X, method = 'equi')$Xk
  }
  else{
    D <- diag(create.solve_sdp(t(X) %*% X) + rnorm(ncol(X), mean = 0, sd = .01))
    X_ko <- create.fixed(X, method = 'sdp')$Xk
  }
...
```{r}
sarkar_tang_method1 <- function(X, Y, alpha = .05, equicor = TRUE){
  d <- ncol(X)
  nu <- nrow(X) - 2 * d # n - 2d assuming n > 2d
  T1 <- get_T_1(X, Y, equicor = equicor)
  T2 <- get_T_2(X, Y, equicor = equicor)
...
```{r}
reg <- get_regression_model(200, 40, 2, num_true_signals = 8)
condition_number_test_sdp(reg$X)
solve(t(reg$X) %*% reg$X)
```
...
result_vec_power_method1_setting1 <- c()
result_vec_fdr_method1_setting1 <- c()
for (amp in c(2, 4, 6, 8, 10)){
  result <- run_method1_simulation(n = 200,
                                          ...
                                          num_true_sigs = 8, 
                                          num_iter = 500)
  result_vec_power_method1_setting1 <- c(result_vec_power_method1_setting1, result$power)
  result_vec_fdr_method1_setting1<- c(result_vec_fdr_method1_setting1, result$fdr)
}
...
```{r}
method1_setting1 <- data.frame(a = c(2, 4, 6, 8, 10), 
                                         power = result_vec_power_method1_setting1, 
                                         fdr = result_vec_fdr_method1_setting1)
```
...
result_vec_power_method1_setting2 <- c()
result_vec_fdr_method1_setting2 <- c()
for (amp in c(2, 4, 6, 8, 10)){
  result <- run_method1_simulation(n = 500,
                                          ...
                                          num_true_sigs = 10, 
                                          num_iter = 500)
  result_vec_power_method1_setting2 <- c(result_vec_power_method1_setting2, result$power)
  result_vec_fdr_method1_setting2<- c(result_vec_fdr_method1_setting2, result$fdr)
}
...
```{r}
method1_setting2 <- data.frame(a = c(2, 4, 6, 8, 10), 
                                         power = result_vec_power_method1_setting2, 
                                         fdr = result_vec_fdr_method1_setting2)
```
...
result_vec_power_method1_setting3 <- c()
result_vec_fdr_method1_setting3 <- c()
for (amp in c(2, 4, 6, 8, 10)){
  result <- run_method1_simulation(n = 1000,
                                          ...
                                          num_true_sigs = 20, 
                                          num_iter = 500)
  result_vec_power_method1_setting3 <- c(result_vec_power_method1_setting3, result$power)
  result_vec_fdr_method1_setting3 <- c(result_vec_fdr_method1_setting3, result$fdr)
}

```

```{r}
method1_setting3 <- data.frame(a = c(2, 4, 6, 8, 10), 
                                         power = result_vec_power_method1_setting3, 
                                         fdr = result_vec_fdr_method1_setting3)
```
...
```{r}
# Setting 1 Power
ggplot(method1_setting1, aes(x = a, y = power)) + 
  geom_line(color = 'red') +
...
```{r}
# Setting 1 FDR
ggplot(method1_setting1, aes(x = a, y = fdr)) + 
  geom_line(color = 'red') +
  geom_point(shape = 4, color = 'red') + 
...
```{r}
# Setting 2 power
ggplot(method1_setting2, aes(x = a, y = power)) + 
  geom_line(color = 'red') +
  geom_point(shape = 4, color = 'red') + 
  scale_y_continuous(limits = c(0, 1)) +
...
```{r}
# Setting 2 FDR
ggplot(method1_setting2, aes(x = a, y = fdr)) + 
  geom_line(color = 'red') +
  geom_point(shape = 4, color = 'red') + 
  scale_y_continuous(limits = c(0, .15)) +
...
```{r}
# Setting 3 power
ggplot(method1_setting3, aes(x = a, y = power)) + 
  geom_line(color = 'red') +
  geom_point(shape = 4, color = 'red') + 
  scale_y_continuous(limits = c(0, 1)) +
  labs(x = 'Signal Strength - Amplitude', y = 'Simulated Power', title = 'Setting 3 Results (n = 1000, d = 100, k = 20)')
```


```{r}
# Setting 3 FDR
ggplot(method1_setting3, aes(x = a, y = fdr)) + 
  geom_line(color = 'red') +
  geom_point(shape = 4, color = 'red') + 
  scale_y_continuous(limits = c(0, .15)) +
  labs(x = 'Signal Strength - Amplitude', y = 'Simulated FDR', title = 'Setting 3 Results (n = 1000, d = 100, k = 20)')
```
...
set.seed(12)
# running setting 3 as specified in the paper
result_vec_power_independent_BH_setting3 <- c()
result_vec_fdr_independent_BH_setting3 <- c()
for (amp in c(2, 4, 6, 8, 10)){
  result <- run_independent_BH_simulation(n = 1000,
                                          d = 100,
                                          a = amp, 
                                          num_true_sigs = 20, 
                                          num_iter = 500)
  result_vec_power_independent_BH_setting3 <- c(result_vec_power_independent_BH_setting3, result$power)
  result_vec_fdr_independent_BH_setting3 <- c(result_vec_fdr_independent_BH_setting3, result$fdr)
}

```

```{r}
independent_BH_setting3 <- data.frame(a = c(2, 4, 6, 8, 10), 
                                         power = result_vec_power_independent_BH_setting3, 
                                         fdr = result_vec_fdr_independent_BH_setting3)
```


## Graphs for Independent BH
