...
get_T_1 <- function(X, Y){
  sigma <- t(X) %*% X
  D <- diag(create.solve_sdp(sigma)) # need to turn the output into a diagonal matrix
  ...
  T_1 <- (1 / get_variance_parameter_estimate(cbind(X, X_ko), Y) * sqrt(2)) *
    sqrtm(solve(diag(diag(2 * sigma - D))))$Binv %*% beta_hat_1 # unsure about interpretation of diag operator in the equation for T1 from the paper; this is my best interpretation of it - Ignat agrees. Binv returns the inverse of the square root that sqrtm calculates, which is what the formula requires.
  #return(D)
  #return(2 * sigma - D)
  return(T_1)
}
...
X <- normc(X, center = F) # X ABSOLUTELY MUST be normalized to get sensical values from the knockoff functions. Here using the normalizing function used internally in the package
