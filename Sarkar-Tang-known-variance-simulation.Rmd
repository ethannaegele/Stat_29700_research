---
title: "Sarkar-Tang-known-var-sims"
author: "Ethan Naegele"
date: "2024-07-11"
output: pdf_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```



```{r}
run_method1_sigma_simulation <- function(n, d, a, num_true_sigs, sigma = 1, multiplier = 1.9, num_iter = 300){
  # Sets up the regression problem with specified dimensions and number of true signals, all of which have
  # the same amplitude. 
  fdp_vec <- numeric(num_iter)
pwr_vec <- c(num_iter)
for (m in 1:num_iter){
  feature_indices <- 1:d
  regression <- get_regression_model(n = n, d = d, a = a, num_true_signals = num_true_sigs)
  true_nonzero_indices <- regression$nonzero_indices
  rejected_indices <- sarkar_tang_method1_sigma(regression$X, regression$Y, alpha = .05, sigma = sigma, multiplier = multiplier)
  num_rejections <- length(rejected_indices)
  num_correct_rejections <- length(intersect(rejected_indices, true_nonzero_indices))
  num_false_rejections <- length(intersect(feature_indices[-true_nonzero_indices], rejected_indices))
  fdp_vec[m] <- num_false_rejections / max(num_rejections, 1)
  pwr_vec[m] <- num_correct_rejections / max(length(true_nonzero_indices), 1)
}
fdr <- mean(fdp_vec)
pwr <- mean(pwr_vec)
return(list(fdr = fdr, power = pwr))
}
```

```{r}
run_sarkar_tang_method1_sigma_setting1_simulation <- function(a_vec = c(2, 4, 6, 8, 10), 
                                                              sigma = 1,
                                                              multiplier = 1.9,
                                               num_iter = 500){
result_vec_power <- c()
result_vec_fdr <- c()
for (amp in a_vec){
  result <- run_method1_sigma_simulation(n = 200,
                                          d = 40,
                                          a = amp, 
                                          num_true_sigs = 8,
                                         sigma = sigma,
                                         multiplier = multiplier,
                                          num_iter = num_iter)
  result_vec_power <- c(result_vec_power, result$power)
  result_vec_fdr <- c(result_vec_fdr, result$fdr)
}
return(data.frame(a = a_vec, power = result_vec_power, fdr = result_vec_fdr))
}
```

```{r}
run_sarkar_tang_method1_sigma_setting2_simulation <- function(a_vec = c(2, 4, 6, 8, 10), 
                                                              sigma = 1,
                                                              multiplier = 1.9,
                                               num_iter = 500){
result_vec_power <- c()
result_vec_fdr <- c()
for (amp in a_vec){
  result <- run_method1_sigma_simulation(n = 500,
                                          d = 50,
                                          a = amp, 
                                          num_true_sigs = 10,
                                         sigma = sigma,
                                         multiplier = multiplier,
                                          num_iter = num_iter)
  result_vec_power <- c(result_vec_power, result$power)
  result_vec_fdr <- c(result_vec_fdr, result$fdr)
}
return(data.frame(a = a_vec, power = result_vec_power, fdr = result_vec_fdr))
}
```

```{r}
run_sarkar_tang_method1_sigma_setting3_simulation <- function(a_vec = c(2, 4, 6, 8, 10), 
                                                              sigma = 1,
                                                              multiplier = 1.9,
                                               num_iter = 500){
result_vec_power <- c()
result_vec_fdr <- c()
for (amp in a_vec){
  result <- run_method1_sigma_simulation(n = 1000,
                                          d = 100,
                                          a = amp, 
                                          num_true_sigs = 20,
                                         sigma = sigma,
                                         multiplier = multiplier,
                                          num_iter = num_iter)
  result_vec_power <- c(result_vec_power, result$power)
  result_vec_fdr <- c(result_vec_fdr, result$fdr)
}
return(data.frame(a = a_vec, power = result_vec_power, fdr = result_vec_fdr))
}
```

```{r}
set.seed(12)
method1_setting1_sigma <- run_sarkar_tang_method1_setting1_simulation()
```

```{r}
set.seed(12)
method1_setting1_sigma_v4 <- run_sarkar_tang_method1_sigma_setting1_simulation()
```



