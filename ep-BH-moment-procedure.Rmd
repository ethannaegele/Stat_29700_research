...
```{r}
ep_moment_method1_3_sigma <- function(X, Y, alpha = .05, sigma = 1, moment = 2, dampen = FALSE){
  # In this setting, sigma is known
  # check that moment is an even integer
  if (!is.numeric(moment) | moment %% 2 != 0 | moment <= 0) {
    stop("Moment must be a positive even integer.")
...
ep_moment_method <- function(X, Y, alpha = .05, moment = 2, dampen = FALSE){
  # check that moment is an even integer
  if (!is.numeric(moment) | moment %% 2 != 0 | moment <= 0) {
    stop("Moment must be a positive even integer.")
  }
...
```{r}
ep_moment_method_alternate <- function(X, Y, alpha = .05, moment = 2, dampen = FALSE){
  ...
  X_ko <-  create_equicorrelated_mult(X, multiplier = 1.98)$Xk
  sigma_hat <- get_variance_parameter_estimate(cbind(X, X_ko), Y)
  rejected_indices <- ep_moment_method1_3_sigma(X, Y, alpha = alpha, sigma = sigma_hat, moment = moment, dampen = dampen)
  return(rejected_indices)
}
```
