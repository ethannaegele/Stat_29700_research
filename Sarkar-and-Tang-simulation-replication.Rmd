...
for (m in 1:20) {
  num_true_signals <- 8
  d <- 40
  regression <- get_regression_model(n = 100, d = d, a = 6, num_true_signals = num_true_signals)
  vec <- c(vec, condition_number_test_equi(regression$X))
}
mean(vec)
...
for (m in 1:20) {
  ...
  d <- 40
  regression <- get_regression_model(n = 100, d = d, a = 6, num_true_signals = num_true_signals)
  vec <- c(vec, condition_number_test_sdp(regression$X))
}
mean(vec)
...
```{r}
set.seed(12)
regression <- get_regression_model(n = 200, d = 40, a = 2, num_true_signals = 8)
...
for (m in 1:50) {
  ...
  vec_equi <- c(vec_equi, condition_number_test_equi(regression$X))
  vec_equi_mult <- c(vec_equi_mult, condition_number_test_equi_mult(regression$X, multiplier = 1.98))
}
mean(vec_equi)
mean(vec_equi_mult)
...
We see from using the modified function that the modified results in a much better behaving matrix.
...
set.seed(12)
regression <- get_regression_model(n = 200, d = d, a = 6, num_true_signals = num_true_signals)
equi_solution <- create.solve_equi(t(regression$X) %*% regression$X)
result <- create.fixed(regression$X, method = 'equi')
result_mult <- create_equicorrelated_mult(regression$X, multiplier = 1.98)
```
