...
```{r}
run_bh_setting1_simulation <- function(a_vec = c(2, 4, 6, 8, 10), 
                                                               alpha = .05, 
                                                               num_iter = 500){
  result_vec_power <- c()
  result_vec_fdr <- c()
  for (amp in a_vec){
    result <- run_bh_simulation(n = 200,
                                          d = 40,
                                          a = amp, 
                                          num_true_sigs = 8,
                                          alpha = alpha,
...
run_bh_setting2_simulation <- function(a_vec = c(2, 4, 6, 8, 10), 
                                                               alpha = .05, 
                                                               num_iter = 500){
  result_vec_power <- c()
  result_vec_fdr <- c()
  for (amp in a_vec){
    result <- run_bh_simulation(n = 500,
                                          d = 50,
                                          a = amp, 
                                          num_true_sigs = 10,
                                          alpha = alpha,
...
```{r}
# not exactly the simulation used in the LFL paper, due to computation concerns, but similar
run_bh_setting3_simulation <- function(a_vec = c(2, 4, 6, 8, 10), 
                                                               alpha = .05, 
                                                               num_iter = 500){
  result_vec_power <- c()
  result_vec_fdr <- c()
  for (amp in a_vec){
    result <- run_bh_simulation(n = 1000,
                                          d = 100,
                                          a = amp, 
                                          num_true_sigs = 20,
                                          alpha = alpha,
