...
```{r}
run_sarkar_tang_method2_mcc_simulation <- function(type = 'MCC', n, d, a, num_true_sigs, alpha = .05, num_iter = 500){
  # Sets up the regression problem with specified dimensions and number of true signals, all of which have
  # the same amplitude. 
  fdp_vec <- numeric(num_iter)
pwr_vec <- c(num_iter)
for (m in 1:num_iter){
  ...
  regression <- get_mcc_regression_model(type = type, n = n, d = d, a = a, num_true_signals = num_true_sigs)
  true_nonzero_indices <- regression$nonzero_indices
  rejected_indices <- sarkar_tang_method2(regression$X, regression$Y, alpha = alpha)
  num_rejections <- length(rejected_indices)
  num_correct_rejections <- length(intersect(rejected_indices, true_nonzero_indices))
  num_false_rejections <- length(intersect(feature_indices[-true_nonzero_indices], rejected_indices))
  fdp_vec[m] <- num_false_rejections / max(num_rejections, 1)
...
```{r}
run_dbh_mcc_simulation <- function(type = 'MCC', n, d, a, num_true_sigs, alpha = .05, num_iter = 500){
  # Sets up the regression problem with specified dimensions and number of true signals, all of which have
  # the same amplitude. 
  fdp_vec <- numeric(num_iter)
pwr_vec <- c(num_iter)
...
```{r}
run_sarkar_tang_method2_mcc_lfl_setting_simulation <- function(type = 'MCC', a_vec = c(2, 4, 6, 8, 10), 
                                                               alpha = .05, 
                                                               num_iter = 500){
  # Ensure alpha is a vector
  if (is.numeric(alpha) && length(alpha) == 1) {
  ...
  for (amp in a_vec){
    for (alp in alpha){
      result <- run_sarkar_tang_method2_mcc_simulation(type = type,
                                                       n = 1000,
                                                       d = 100,
                                                       a = amp, 
                                                       num_true_sigs = 10,
...
```{r}
run_dbh_mcc_lfl_setting_simulation <- function(type = 'MCC', 
                                                      a_vec = c(2, 4, 6, 8, 10), 
                                                      alpha = .05,
                                                      num_iter = 500){
  # Ensure alpha is a vector
  ...
  for (amp in a_vec){
    for (alp in alpha){
      result <- run_dbh_mcc_simulation(type = type,
                                                       n = 1000,
                                                       d = 100,
                                                       a = amp,
                                                       num_true_sigs = 10,
