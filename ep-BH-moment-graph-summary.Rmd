...
```{r}
combined_df_setting1 <- method1_setting1_v2 %>% 
  ...
  full_join(ep_moment_method_setting1_m2_v2 %>% rename(power_ep_moment = power, fdr_ep_moment = fdr), by = 'a') %>% 
  full_join(ep_moment_method_alternate_setting1_m2_v2 %>% rename(power_ep_moment_alternate = power, fdr_ep_moment_alternate = fdr), by = 'a') %>%
...
power_plot_summary_setting1 <- ggplot(long_df_setting1_power, aes(x = a, y = Power, color = Method)) +
  ...
  scale_shape_manual(values = c(4, 1, 5, 2, 3, 10, 19)) +
  scale_y_continuous(limits = c(0, 1)) + 
  labs(title = "Simulated Power Across Different Methods - Setting 1 (n = 200, d = 40, k = 8)", x = "Signal Strength - Amplitude", y = "Simulated Power") +
  theme_minimal()
...
```{r}
combined_df_setting2 <- method1_setting2_v2 %>% 
  ...
  full_join(ep_moment_method_setting2_m2_v2 %>% rename(power_ep_moment = power, fdr_ep_moment = fdr), by = 'a') %>% 
  full_join(ep_moment_method_alternate_setting2_m2_v2 %>% rename(power_ep_moment_alternate = power, fdr_ep_moment_alternate = fdr), by = 'a') %>% 
...
```{r}
combined_df_setting3 <- method1_setting3_v2 %>% 
  ...
  full_join(ep_moment_method_setting3_m2_v2 %>% rename(power_ep_moment = power, fdr_ep_moment = fdr), by = 'a') %>% 
  full_join(ep_moment_method_alternate_setting3_m2_v2 %>% rename(power_ep_moment_alternate = power, fdr_ep_moment_alternate = fdr), by = 'a') %>% 
